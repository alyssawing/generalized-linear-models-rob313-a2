...
def load_dataset(name, n_train=None, d=None, print_details=False):
    ...
    if name.lower() == "rosenbrock":
        assert (
            n_train is not None and d is not None
        ), "n and d must be specified for the rosenbrock function"
        ...
        assert (
            n_train is None and d is None
        ), "n and d must only be specified for the rosenbrock function"
        with np.load(
            os.path.join(os.path.dirname(__file__), name.lower() + ".npz")
        ) as data:
    ...
    if name.lower() == "mnist_small":  # convert to float and normalize inputs
        ...
        x_train = x_scaler.fit_transform(np.float64(x_train).reshape((-1, 1))).reshape(
            (10000, 784)
        )
        x_test = x_scaler.transform(np.float64(x_test).reshape((-1, 1))).reshape(
            (1000, 784)
        )
        x_valid = x_scaler.transform(np.float64(x_valid).reshape((-1, 1))).reshape(
            (1000, 784)
        )
